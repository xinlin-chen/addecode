...
%                 [back,time] = progress(k,dims(4),'Combining coil data for echo',back,time);
   ...
   [W,FreqC] = nanMEW(TE(idx),T2,Freq(:,:,:,idx));%nanMEW - do not use empty voxels from Freq; average meanaingul voxels only
    save_nii(make_nii(Freq,vox,[0 0 0],16),'Freq.nii.gz');
    save_nii(make_nii(FreqC,vox,[0 0 0],16),'FreqC.nii.gz');
    save_nii(make_nii(uint8(maskME),vox,[0 0 0],2),'maskME.nii.gz');
    save_nii(make_nii(Mag,vox,[0 0 0],16),'Mag.nii.gz')
